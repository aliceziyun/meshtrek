...
benchmark_hotel() {
    ...
    ~/DeathStarBench/wrk2/wrk -D exp -t "$THREADS" -c "$CONNECTIONS" -d "$DURATION" -L -s ~/meshtrek/setup/benchmark/HotelReserve/wrk2/frontend_normal.lua "http://$request_url" -R $TARGET_RPS | tee -a "$OUTPUT_FILE"
