...
local function search_hotel() 
  ...
  local method = "GET"
  local path = "/new?hotels?inDate=" .. in_date_str .. 
...
local function recommend()
  ...
  local method = "GET"
  local path = "/new?recommendations?require=" .. req_param .. 
...
local function reserve()
  ...
  local method = "POST"
  local path = "/new?reservation?inDate=" .. in_date_str .. 
    "&outDate=" .. out_date_str .. "&lat=" .. tostring(lat) .. "&lon=" .. tostring(lon) ..
...
local function user_login()
  ...
  local method = "POST"
  local path = "/new?user?username=" .. user_name .. "&password=" .. password
  local headers = {}
  -- headers["Content-Type"] = "application/x-www-form-urlencoded"
  return wrk.format(method, path, headers, nil)
end
